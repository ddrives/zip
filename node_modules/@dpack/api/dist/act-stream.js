...
function watchLocal(vault, paths) {
  ...
  function onMetaAppend() {
    ...
    vault.tree._getAndDecode(block, {}, function (err, entry) {
      if (err || !entry) return;
...
function watchRemote(vault, paths) {var onContentDownload = function () {var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(
    ...
    function _callee(block) {var range;return regeneratorRuntime.wrap(function _callee$(_context) {while (1) {switch (_context.prev = _context.next) {case 0:_context.next = 2;return (
                ...
                findEntryByContentBlock(vault, block));case 2:range = _context.sent;
  ...
  vault.metadata.on('download', onMetaDownload);if (vault.content) {wireContent();} else {vault.on('content', wireContent);}function wireContent() {vault.content.on('download', onContentDownload);}stream.on('close', function () {// unlisten events
    vault.metadata.removeListener('download', onMetaDownload);if (vault.content) {vault.content.removeListener('download', onContentDownload);}}); // handlers
  function onMetaDownload(block) {vault.tree._getAndDecode(block, {}, function (err, entry) {if (err || !entry) return; // apply path matching
      ...
      } else {var st = vault.tree._codec.decode(entry.value);var range = { start: st.offset, end: st.offset + st.blocks };isChanged = isDownloaded(vault, range);}if (isChanged) {emitter.emit('changed', { path: entry.name });}});}return stream;}function isDownloaded(vault, range) {if (!vault.content || !vault.content.opened) return false;for (var i = range.start; i < range.end; i++) {
...
function createNetworkActivityStream(vault, path) {
  ...
  if (vault.content) track(vault.content, 'content');else
  vault.on('content', function () {return track(vault.content, 'content');});
